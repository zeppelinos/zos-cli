...
export default async function init({ name, version, stdlibNameVersion = undefined, installLib = false, force = false, packageFile = undefined }) {
  if (name === undefined) throw Error('A project name must be provided to initialize the project.')
  
  const appController = new LocalAppController(packageFile)
